...
os.makedirs(f"{glavna_mapa}/imagesTs", exist_ok=True)       # inferenca
...
for koren in izvorne_mape:
    if os.path.exists(koren):
        slike = glob.glob(f"{koren}/*.img.nii.gz")  # Najdemo vse slike .img.nii.gz
        for slika in slike:
            labela = slika.replace(".img.nii.gz", ".label.nii.gz") # Poiščemo pripadajočo labelo (masko)
            if os.path.exists(labela):
                datoteke.append((slika, labela))

# Sortiranje po številki v imenu 
...
for i in tqdm(range(limit)):
    ...
    # Preimenujemo v nnU-Net format (slika mora imeti končnico _0000)
